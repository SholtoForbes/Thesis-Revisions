...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.8\linewidth]{figures/4_LODESTAR/global}
...
\begin{equation}
\begin{split}
\dot{\gamma} = \frac{(L + T\sin\alpha) \cos\eta}{mv} + (\frac{v}{r}-\frac{g_r}{v})\cos\gamma -\frac{g_t}{v}\sin\gamma\sin\zeta\\ + \cos\phi[2\omega_E \cos\zeta + \frac{\omega_E^2 r}{v}(\cos\phi\cos\gamma+\sin\phi\sin\gamma\sin\zeta)]
\end{split}
\end{equation}
\begin{equation}
\dot{v} = \frac{T\cos\alpha - D}{m}-g_r\sin\gamma +g_t\cos\gamma\sin\zeta
...
\\ - \frac{\omega_E^2 r}{v\cos\gamma}\sin\phi\cos\phi\cos\zeta-2\omega_E\sin\phi +\frac{g_t\cos\zeta}{v\cos\gamma}
\end{split}
\end{equation}
\noindent
...
The rate of heat transfer on the body of the vehicle is computed by initially using Reynolds analogy to determine a Stanton number at each location and flight condition: 
\begin{equation}
c_h = \frac{c_f}{2s},
\end{equation}
where $c_h$ is the Stanton number, $c_f$ is the coefficient of friction and $s$ is the Reynold's analogy factor, assumed to be 1.16\cite{Ward2018}. The Stanton number is then used to calculate the heat transfer coefficient:
\begin{equation}
h = c_h \rho V c_p,
\end{equation}
where $\rho, V$ and $c_p$ are the density, velocity and specific heat of the air respectively. The rate of heat transfer is then calculated as follows:
\begin{equation}
q = h(T_{air}-T_w),
\end{equation}
where $T_{air}$ is the air temperature and $T_w$ is the wall temperature. 
The value of $c_f$ is determined using the viscous solver VC3D\cite{Ward2018}, at a range of flight conditions. Tabulated $c_f$ values were provided for this study by Mr Alexander Ward. 


Reynold's analogy is not valid at the stagnation point and leading edge of the wings due to large pressure gradients in the flow\cite{HypersonicGas}. At these points, the heat transfer is estimated using semi-empirical correlations\cite{Dirkx}. At the stagnation point, the heat transfer is given by\cite{Dirkx}:
\begin{equation}
...
\end{equation}
where $R_n$ is the nose radius, and $T_{aw}$ is the adiabatic wall temperature.
...
Table \ref{tab:thermalprops}, summarises the parameters and modelling assumptions used for the analysis of points shown in Figure \ref{fig:Q_pos}. For all cases the standard optimised trajectory with fly-back is used (Case 11, Section \ref{sec:case11}). 
